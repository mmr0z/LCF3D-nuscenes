...
class KittiConverter:
    def __init__(self,
                 nusc_kitti_dir: str = '/mnt/proj2/dd-24-8/nuscenes_sweeps_dataset_kitti_format',
                 cam_name: str = 'CAM_FRONT',
                 lidar_name: str = 'LIDAR_TOP',
                 image_count: int = 10,
                 nusc_version: str = 'v1.0-trainval',
                 nusc_dataroot: str = '/mnt/proj2/dd-24-8/nuscenes_dataset',
        ...
        self.nusc = NuScenes(version=nusc_version, dataroot=nusc_dataroot, verbose=True)
    ...
    def nuscenes_gt_to_kitti(self) -> None:
        ...
        image_sets_folder = os.path.join(self.nusc_kitti_dir, 'ImageSets')
        for folder in [label_folder, calib_folder, image_folder, lidar_folder, image_sets_folder]:
        ...
        with open(os.path.join(image_sets_folder, self.split + '.txt'), 'w') as f:
            for token_idx in range(len(sample_tokens)):
                f.write('%06d' % token_idx + '\n')
        ...
        for sample_token in tqdm(sample_tokens):
            ...
            if not self.with_sweeps:
                pcl = LidarPointCloud.from_file(src_lid_path)
                pcl.rotate(kitti_to_nu_lidar_inv.rotation_matrix)  # In KITTI lidar frame.
                with open(dst_lid_path, "w") as lid_file:
                    pcl.points.T.tofile(lid_file)
            else:
                pcl, times = LidarPointCloud.from_file_multisweep(self.nusc, sample, self.lidar_name, self.lidar_name, self.n_sweeps)
                pcl.rotate(kitti_to_nu_lidar_inv.rotation_matrix)
                points = pcl.points
                points = np.concatenate([points, times], axis=0)
                with open(dst_lid_path, "w") as lid_file:
                    points.T.tofile(lid_file)
...
if __name__ == '__main__':
    ...
    KittiConverter().nuscenes_gt_to_kitti()