...
from mmdet3d.structures import (CameraInstance3DBoxes, LiDARInstance3DBoxes,
                                bbox3d2result, xywhr2xyxyr, Box3DMode)
...
TP_METRICS_ = ['trans_err', 'scale_err', 'orient_err']


@METRICS.register_module()
class KittiNuScenesMetric(BaseMetric):
    """Nuscenes evaluation metric for kitti dataset.
    ...
    def __init__(self,
        ...
        super(KittiNuScenesMetric, self).__init__(
        ...
        self.eval_detection_configs.class_names = ['car', 'pedestrian', 'bicycle']
        self.eval_detection_configs.mean_ap_weight = 3
    ...
    def compute_metrics(self, results: List[dict]) -> Dict[str, float]:
        ...
        valid_annot_labels = [self.classes.index(cls) for cls in classes]
        # load annotations
        self.data_infos = load(
            self.ann_file, backend_args=self.backend_args)['data_list']
        
        tmp_dir = tempfile.TemporaryDirectory()
        jsonfile_prefix = osp.join(tmp_dir.name, 'annotations')
        tmp_anno_path, sample_tokens = self._convert_annotations(
            self.data_infos, classes, valid_annot_labels, jsonfile_prefix)
        gt_boxes, _ = load_prediction(tmp_anno_path, 10000, DetectionBox, verbose=False)
        
        result_dict, tmp_dir = self.format_results(results, classes, sample_tokens,
        ...
        for metric in self.metrics:
            ap_dict = self.nus_evaluate(
                result_dict, gt_boxes, classes=classes, metric=metric, logger=logger)
    ...
    def nus_evaluate(self,
        ...
        for name in result_dict:
            ...
            ret_dict = self._evaluate_single(
                result_dict[name], gt_boxes, classes=classes, result_name=name)
    ...
    def _evaluate_single(
        ...
        pred_boxes, meta = load_prediction(
            result_path, self.eval_detection_configs.max_boxes_per_sample, 
        ...
        for name in classes:
            name = self.kitti2nusc_mapping[name]
            ...
            if 'pred' in name and '3d' in name and name[0] != '_':
                ...
                if box_type_3d == LiDARInstance3DBoxes:
                    result_dict[name] = self._format_lidar_bbox(
                        results_, sample_idx_list, sample_tokens, classes, tmp_file_)
                elif box_type_3d == CameraInstance3DBoxes:
                    raise NotImplementedError(
                        'CameraInstance3DBoxes is not supported yet')
    ...
    def _convert_annotations(self,
                             ...
                             classes: Optional[List[str]] = None,
                             valid_annot_labels: Optional[List[int]] = None,
                             jsonfile_prefix: Optional[str] = None) -> str:
        nusc_annos = {}
        sample_idx_list = [info['sample_idx'] for info in annotations]
        
        print('Start to convert detection format...')
        for i, info in enumerate(mmengine.track_iter_progress(annotations)):
            sample_idx = info['sample_idx']
            
            if len(info['instances']) == 0:
                nusc_annos[str(info['sample_idx'])] = []
                continue
            
            ann_info = self.parse_ann_info(info)
            lidar2cam = np.array(info['images']['CAM2']['lidar2cam'])
            # convert gt_bboxes_3d to velodyne coordinates with `lidar2cam`
            filter_instances = np.isin(ann_info['gt_labels_3d'], valid_annot_labels)
            gt_bboxes_3d = CameraInstance3DBoxes(
                ann_info['gt_bboxes_3d'][filter_instances]
                ).convert_to(Box3DMode.LIDAR, np.linalg.inv(lidar2cam))
            boxes, attrs = output_to_nusc_box_no_velocity({
                'bboxes_3d': gt_bboxes_3d,
                'scores_3d': torch.ones_like(gt_bboxes_3d.tensor[:, 0]),
                'labels_3d': torch.tensor(ann_info['gt_labels_3d'][filter_instances], dtype=torch.long),
            })
            
            # skipping as other datasets (e.g. kitti) use lidar coordinates
            ...
            for i, box in enumerate(boxes):
                try:
                    name = self.kitti2nusc_mapping[classes[box.label]]
                except IndexError as e:
                    print(f'Classes: {classes}, label: {box.label}')
                    raise e
            ...
            nusc_annos[str(sample_idx)] = annos
        nusc_submissions = {
            'meta': self.modality,
            'results': nusc_annos,
        }
        ...
        return res_path, sample_idx_list
    ...
    def _format_lidar_bbox(self,
        ...
        for i, det in enumerate(mmengine.track_iter_progress(results)):
            annos = []
            boxes, attrs = output_to_nusc_box_no_velocity(det)
            sample_idx = sample_tokens[sample_idx_list[i]]
            # skipping as other datasets (e.g. kitti) use lidar coordinates
            # boxes = lidar_nusc_box_to_global(self.data_infos[sample_idx],
            #                                  boxes, classes,
            #                                  self.eval_detection_configs)
        ...
        mmengine.mkdir_or_exist(jsonfile_prefix)
        res_path = osp.join(jsonfile_prefix, 'annotations_nusc.json')
        print(f'Results writes to {res_path}')
        mmengine.dump(nusc_submissions, res_path)
        return res_path


def output_to_nusc_box_no_velocity(
    ...
    if isinstance(bbox3d, LiDARInstance3DBoxes):
        # filter bboxes that are not in range [-50, 50] of x coordinate
        # (since for kitti the range is already [-40, 40])
        x_ = box_gravity_center[:, 0]
        filter_bboxes = np.logical_and(x_ >= -50, x_ <= 50)
        bbox3d = bbox3d[filter_bboxes]
        box_gravity_center = box_gravity_center[filter_bboxes]
        box_dims = box_dims[filter_bboxes]
        box_yaw = box_yaw[filter_bboxes]
        labels = labels[filter_bboxes]
        scores = scores[filter_bboxes]
        if attrs is not None:
            attrs = attrs[filter_bboxes]
...
class DetectionMetrics:
    ...
    See Towards Domain Generalization for Multi-view 3D Object Detection in Bird-Eye-View for NDS variant.
    https://arxiv.org/pdf/2303.01686
    """
    ...
    def tp_errors(self) -> Dict[str, float]:
        ...
        errors = {}
        for metric_name in TP_METRICS_:
            class_errors = []
...
class CustomNuScenesDetectionEval:
    """
