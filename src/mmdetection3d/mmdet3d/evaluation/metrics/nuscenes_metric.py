...
class NuScenesMetric(BaseMetric):
    ...
    def _evaluate_single(
        ...
        if self._results_are_empty(result_path):
            logger: MMLogger = MMLogger.get_current_instance()
            logger.warning(
                'nuScenes evaluation skipped because no predicted boxes '
                f'remained after formatting/filtering in {result_path}. '
                'Returning zero metrics for this result set.')
            return self._empty_metrics(classes, result_name)
        ...
        try:
            ...
            if 'Invalid box type: None' not in str(exc):
                raise
            logger: MMLogger = MMLogger.get_current_instance()
            logger.warning(
                'nuScenes devkit rejected the formatted predictions with '
                '`Invalid box type: None`, which indicates that no valid '
                'predicted boxes were available for evaluation. Returning '
                'zero metrics for this result set.')
            return self._empty_metrics(classes, result_name)
    ...
    def _empty_metrics(self, classes: Optional[List[str]],
                       result_name: str) -> Dict[str, float]:
        """Build a zero-valued metric dict for empty prediction sets."""
        detail = dict()
        metric_prefix = f'{result_name}_NuScenes'
        classes = classes or []
        for name in classes:
            for dist_th in self.eval_detection_configs.dist_ths:
                detail[f'{metric_prefix}/{name}_AP_dist_{dist_th}'] = 0.0
            for metric_name in self.ErrNameMapping.values():
                detail[f'{metric_prefix}/{name}_{metric_name}'] = 0.0
        for metric_name in self.ErrNameMapping.values():
            detail[f'{metric_prefix}/{metric_name}'] = 0.0
        detail[f'{metric_prefix}/NDS'] = 0.0
        detail[f'{metric_prefix}/mAP'] = 0.0
        return detail

    @staticmethod
    def _results_are_empty(result_path: str) -> bool:
        """Return True when the formatted nuScenes submission has no boxes."""
        with open(result_path) as f:
            result_data = json.load(f)
        results = result_data.get('results', {})
        return not any(len(sample_boxes) > 0 for sample_boxes in results.values())
